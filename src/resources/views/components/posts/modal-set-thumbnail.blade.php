<div x-show="modalSetThumbnail" class="fixed inset-0 z-50 bg-gray-600 bg-opacity-50 h-full w-full">
    <div class="relative top-10 mx-auto p-2 border w-4/5 h-5/6 shadow-lg rounded-md bg-white overflow-y-auto"
         x-transition:enter="ease-out duration-300"
         x-transition:enter-start="opacity-0 scale-90"
         x-show="modalSetThumbnail"
         x-transition:enter-end="opacity-100 scale-100"
         x-transition:leave="ease-in duration-300"
         x-transition:leave-start="opacity-100 scale-100"
         x-transition:leave-end="opacity-0 scale-90"
         @click.outside="modalSetThumbnail = false;">
        <div class="flex">
            <h1 class="text-lg w-full ml-2 text-gray-semparar">Definir thumbnail da postagem</h1>

            <div class="flex w-full justify-end text-2xl px-2">
                <button @click="modalSetThumbnail = false;" type="button">
        ...
        <div class="px-2 pt-2">
            ...
            <div class="sticky -bottom-2 bg-white flex justify-end w-full pb-4 p-4" x-show="selectedImage != null" x-transition>
                <x-button type="button"
                          class="text-white bg-green-500 hover:bg-green-600"
                          @click="
                            modalSetThumbnail = false;
                            selectedImage = null;
                            thumbnail = selectedImageInformation.path;">
                    Definir imagem
                </x-button>
            </div>
        </div>
