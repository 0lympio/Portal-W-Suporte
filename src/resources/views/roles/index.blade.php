<x-app-layout>
    <div x-data="dataTable({{ collect($roles) }})" x-cloak>
        <div>
            <x-title>
                <x-slot name="title">Lista de perfis</x-slot>
                <x-slot name="buttons">
                        ...
                        <template x-for="(item, index) in items" :key="index">
                            <tr x-show="checkView(index + 1)" class="hover:bg-gray-200 text-gray-900 text-xs">
                                ...
                                @canany(['roles.edit', 'roles.destroy'])
                                    <td class="actions">
                                        @can(['roles.edit'])
                                            <a :href="`roles/${item.id}/edit`">
                                                <x-datatable.button class="fa fa-edit text-blue" />
                                            </a>
                                        @endcan
                                        @can(['roles.destroy'])
                                            <x-datatable.button @click="deleteModal=true;deleteUrl=`roles/${item.id}`"
        ...
        <x-delete-modal>Tem certeza que deseja deletar esse Perfil?</x-delete-modal>
    </div>
    <script>
