<x-app-layout>
    ...
    <div class="mb-4">
        <h1 class="text-2xl font-bold text-gray-semparar">Editar perfil</h1>
    ...
    <div class="flex w-full" x-data="accordion({{ collect($permissions) }}, {{ collect($role->permissions->pluck('id', 'name')) }})" x-init="initData()">
        <form class="bg-white py-4 w-full px-3" method="POST" action="{{ route('roles.update', $role->id) }}">
            @csrf
            @method('PUT')
            <div class="mt-4">
                <x-label for="name" value="Nome para o perfil" />
                <x-input id="name" class="input-form" type="text" name="name" required autofocus
                    maxlength="255" value="{{ old('name', $role->name) }}" />
            </div>
            ...
            <div class="mt-4 mb-4">
                <div class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                    ...
                    <x-label for="all_permission"
                        class="toggle-label block overflow-hidden h-6 rounded-full bg-gray-300 cursor-pointer">
                    </x-label>
                </div>
                <x-label for="toggle" class="text-xs text-gray-700"><span>Selecionar todas permissões</span></x-label>
            </div>

            <div class="w-full border border-gray-200">
                <ul class="shadow-box">
                    <template x-data="" x-for="(item, index) in items">
                                            ...
                                            <div class="m-6 justify-center">
                                                <div
                                                    ...
                                                    <input type="checkbox" :name="'categories[' + category.slug + ']'"
                                                        :id="category.name"
                                                        :checked="!!rolePermissions[category.slug]"
                                        ...
                                        <div class="flex items-center m-6 justify-center float-left">
                                            <div
                                                ...
                                                <input type="checkbox" :name="'permission[' + permission.id + ']'"
                                                    :id="permission.name"
                                                    :checked="!!rolePermissions[permission.value]"
                                                    class="permission input-checkbox-switch" />
                                                <label :for="permission.name"
                                                    class="toggle-label block overflow-hidden h-6 rounded-full bg-gray-300 cursor-pointer"></label>
                                            </div>
                                            <x-label for="toggle" class="text-xs text-gray-700 capitalize"><span
                                                    x-text="$t(permission.name)"></span></x-label>
                                        </div>
                                    </template>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        window.accordion = function(data, rolePermissions) {
            return {
                items: [],
                selected: null,
                rolePermissions,
